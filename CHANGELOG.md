...
### Added
- **Calendar provider config guards + capabilities endpoint** — the Google/Outlook OAuth connect routes now return a clean `503` (with a "not available on this server" message) **before any DB write** when their OAuth credentials aren't configured, instead of creating an orphan `disconnected` connection row and then throwing a cryptic `500` when `generateAuthUrl()`/`createOAuth2Client()` fails on the missing config. New `isConfigured()` on each OAuth service backs the guard. New `GET /api/calendar/capabilities` (auth-gated) reports `{ google, outlook, apple, ics, cozi }` so the calendar settings UI hides unconfigured providers behind an "Unavailable" chip rather than a dead "Connect" button (Apple/ICS/Cozi use user-supplied credentials and are always available). +7 tests. **Outlook remains parked** pending a Microsoft Azure AD app registration — once `MICROSOFT_CLIENT_ID/SECRET/REDIRECT_URI` are set, it lights up with no code change. Full rationale + OAuth provisioning steps (Azure AD + Google Cloud) + parked-state resume guide: [ADR 0022](docs/decisions/0022-external-calendar-integration-and-config-guards.md). Commented keys added to `.env.example`.
...
### Fixed
- **Calendar sync engine — 7 logic bugs from an end-to-end audit** (8 June) — after restoring the OAuth/token layer, a full logic audit of the sync flow found and fixed: **(#1, HIGH)** Outlook was never in the sync-engine provider switch, so connected Outlook accounts never synced (fell through to "Unknown provider") — now routed to `outlookCalendarService.syncCalendar`. **(#4, HIGH)** ICS import wrote non-existent columns `is_all_day`/`source`/`metadata` (real columns: `all_day`/`external_source`), so every ICS event insert silently threw — ICS feed + file import were fully broken. **(#3, HIGH)** ICS hard-deleted Rowan events on any feed shortfall (transient short feed or the 1000-event cap) → permanent data loss; now soft-deletes like the other providers. **(#2, HIGH)** Outlook delta `@removed` deleted the mapping but orphaned the Rowan event; now soft-deletes the event too. **(#5, MED)** Apple deletions never propagated — iCloud hard-deletes events (the .ics vanishes) rather than emitting CANCELLED, so the CANCELLED-only path never fired; now detects feed-vanished events on a full sync and soft-deletes them. **(#6, MED)** echo loop — `processOutboundCreate` (Google + Apple) didn't check for an existing mapping, so an inbound-synced event got pushed back out as a duplicate; added a per-connection echo guard. **(#7, MED)** Apple conflict detection derived `externalUpdated` from `mapICalendarToRowan().updated_at` (never set) → always "now" → conflicts over-reported; now uses the real iCal `LAST-MODIFIED`. +2 regression tests (Outlook routing, ICS columns); 56/56 calendar tests green. Config note: Outlook also needs `MICROSOFT_CLIENT_ID/SECRET/REDIRECT_URI` set in the environment (Azure AD app) — missing locally.
