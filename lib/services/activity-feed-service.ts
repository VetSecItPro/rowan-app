  ...
  async getRecentActivities(spaceId: string, limit: number = 10): Promise<ActivityItem[]> {
    ...
    try {
      ...
      const [tasks, goals, messages, events, checkIns, shoppingLists, meals, expenses, projects, reminders, chores] = await Promise.all([
        ...
        supabase.from('shopping_lists').select('id, name, created_by, created_at, updated_at, users!shopping_lists_created_by_fkey(id, name, email, avatar_url)').eq('space_id', spaceId).order('created_at', { ascending: false }).limit(5),
