...
export const choreRotationService = {
  async createRotation(choreId: string, userIds: string[], frequency: string, rotationType: string, createdBy: string): Promise<any> {
  ...
  async getRotation(choreId: string): Promise<any> {
    const supabase = createClient();
  ...
  async updateRotation(rotationId: string, updates: any): Promise<any> {
    const supabase = createClient();
