...
export async function countTodaysUserMessages(
  ...
  // nosemgrep: supabase-missing-space-id-filter - the free AI daily cap is per-USER across all their spaces (3 messages/day total, not 3 per space), so this intentionally filters by user (via the ai_conversations FK), not space_id.
