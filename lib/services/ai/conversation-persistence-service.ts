...
/**
 * Phase 10.7: free-tier AI teaser hard cap. A free user gets this many AI
 * messages per day, then is nudged to subscribe. Enforcement is an ATOMIC Redis
 * limiter (checkFreeAIDailyLimit in lib/ratelimit.ts) - keep this value in sync
 * with that limiter's window count. Paid tiers are NOT message-capped (their
 * token + monthly-COGS budgets bound cost).
 */
