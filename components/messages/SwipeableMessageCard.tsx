...
export function SwipeableMessageCard({
  ...
  // Both swipe-revealed action buttons are positioned absolute behind the
  // swipeable content. opacity-0 alone is not enough — Playwright's
  // isVisible() considers an element with opacity:0 still visible, and
  // screen readers may still announce it. Use visibility + aria-hidden +
  // tabIndex to gate interactability on actual swipe state.
  const isEditRevealed = offsetX > EDIT_THRESHOLD / 3;
  const isDeleteRevealed = offsetX < DELETE_THRESHOLD / 3;
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {onEdit && isOwn && (
        <div
          className="absolute inset-y-0 left-0 w-16 flex items-center justify-start pl-2 z-20"
          style={{ visibility: isEditRevealed ? 'visible' : 'hidden' }}
          aria-hidden={!isEditRevealed}
        >
          <div className={`transition-all duration-300 ${isEditRevealed ? 'scale-100 opacity-100' : 'scale-75 opacity-0'}`}>
            <button aria-label="Edit"
              onClick={handleEdit}
              tabIndex={isEditRevealed ? 0 : -1}
      ...
      {onDelete && (
        <div
          className="absolute inset-y-0 right-0 w-20 flex items-center justify-end pr-2 z-20"
          style={{ visibility: isDeleteRevealed ? 'visible' : 'hidden' }}
          aria-hidden={!isDeleteRevealed}
        >
          <div className={`transition-all duration-300 ${isDeleteRevealed ? 'scale-100 opacity-100' : 'scale-75 opacity-0'}`}>
