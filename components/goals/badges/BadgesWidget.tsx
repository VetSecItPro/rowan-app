...
export default function BadgesWidget({ userId, spaceId }: BadgesWidgetProps) {
  ...
  if (loading) {
    return (
      <div className="bg-gray-800 border border-gray-700 rounded-xl p-4 sm:p-6 h-full animate-pulse">
    ...
    <div className="bg-gradient-to-br from-amber-900/20 to-yellow-900/20 border border-amber-800 rounded-xl p-4 sm:p-6 hover:shadow-lg transition-shadow">
      ...
      {stats.totalBadges === 0 ? (
        ...
        <>
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                {recentBadges.map((userBadge) => (
                  <div
                    key={userBadge.id}
                    className="flex-1 bg-gray-800 rounded-lg p-2 border border-amber-800 hover:shadow-md transition-shadow"
                    title={userBadge.badge?.name}
                  >
                    <div className="text-2xl text-center">{userBadge.badge?.icon}</div>
                    <div className="text-xs text-center text-gray-400 truncate mt-1">
          ...
          <Link
            href="/goals/badges"
            className="mt-3 block text-center text-sm text-amber-400 hover:text-amber-300 font-medium transition-colors"
          >
            View All Badges →
          </Link>
        </>
