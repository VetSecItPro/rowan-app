...
export function NewChoreModal({ isOpen, onClose, onSave, editChore, spaceId }: NewChoreModalProps) {
  ...
  useEffect(() => {
    if (editChore) {
      setFormData({ space_id: spaceId, title: editChore.title, description: editChore.description || '', frequency: editChore.frequency, status: editChore.status, due_date: editChore.due_date || '' });
    } else {
      setFormData({ space_id: spaceId, title: '', description: '', frequency: 'weekly', status: 'pending', due_date: '' });
    }
  }, [editChore, spaceId]);
