...
export function WeeklyInsights({ checkIns }: WeeklyInsightsProps) {
  ...
  const moodColors: Record<string, { bg: string; border: string; text: string }> = {
    great: { bg: 'bg-green-900/30', border: 'border-green-500', text: 'text-green-300' },
    good: { bg: 'bg-blue-900/30', border: 'border-blue-500', text: 'text-blue-300' },
    okay: { bg: 'bg-yellow-900/30', border: 'border-yellow-500', text: 'text-yellow-300' },
    meh: { bg: 'bg-orange-900/30', border: 'border-orange-500', text: 'text-orange-300' },
    rough: { bg: 'bg-red-900/30', border: 'border-red-500', text: 'text-red-300' },
  };
  ...
  return (
    <div className="bg-gradient-to-br from-purple-900/30 via-pink-900/20 to-blue-900/30 rounded-xl p-6 border border-purple-500/20 space-y-6">
      ...
      {/* Mood Distribution */}
      <div>
        <h4 className="text-sm font-semibold text-gray-300 mb-3">Mood Distribution</h4>
          ...
          {Object.entries(summary.moodDistribution).map(([mood, count]) => {
            ...
            return (
              <div key={mood} className="flex items-center gap-2">
                <span className="text-xs font-medium text-gray-400 w-14 capitalize">
                  {mood}
                </span>
        ...
        {summary.dominantMood && (
          <div className="mt-3 text-center">
            <p className="text-xs text-gray-400">
              Most common mood: <span className="font-semibold capitalize text-white">{summary.dominantMood}</span>
            </p>
          </div>
        )}
      </div>
        ...
        <div>
          ...
          <div className="space-y-2">
            {patterns.map((pattern, index) => (
              <div
                key={index}
                className="p-3 rounded-lg bg-purple-900/20 border border-purple-800"
              >
                <div className="flex items-start gap-2">
                  <Sparkles className="w-4 h-4 text-purple-400 mt-0.5 flex-shrink-0" />
                  <div className="flex-1">
                    <p className="text-xs text-purple-100">{pattern.description}</p>
                  </div>
