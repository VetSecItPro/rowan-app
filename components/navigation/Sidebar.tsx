      ...
      >
        ...
        <div
          className={`relative flex-shrink-0 w-8 h-8 xl:w-10 xl:h-10 rounded-lg ${item.gradient} flex items-center justify-center shadow-lg transition-all duration-200 group-hover:scale-110 group-hover:shadow-xl ${
            isActive ? 'shadow-2xl ring-2 ring-white/20 scale-105' : 'shadow-md'
          }`}
          style={{
            boxShadow: isActive
...
export function Sidebar() {
  ...
  if (!mounted) {
    return (
      <aside className="hidden md:flex flex-col h-screen sticky top-0 w-64 bg-gray-900/60 border-r border-gray-800/30" />
    );
  }
    ...
    >
      ...
      <div className={`flex items-center h-14 px-3 border-b border-gray-700/30 ${effectivelyExpanded ? 'justify-between' : 'justify-center'}`}>
        ...
        <button
          ...
          className="p-1.5 rounded-lg text-gray-400 hover:text-gray-300 hover:bg-gray-800 transition-all duration-200"
      ...
      <div className="h-8 bg-gradient-to-t from-gray-900/80 to-transparent pointer-events-none" />
    </aside>
  );
}
