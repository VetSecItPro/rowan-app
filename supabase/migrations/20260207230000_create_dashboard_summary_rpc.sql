-- Create optimized dashboard summary RPC
-- Replaces 18+ individual queries with single aggregated call
-- Returns flat JSONB structure matching useDashboardStats.ts expectations
...
BEGIN
    ...
    v_result := jsonb_set(v_result, '{tasks}', (
        SELECT jsonb_build_object(
            ...
            'overdue', COUNT(*) FILTER (WHERE due_date < v_today AND status != 'completed'),
    ...
    v_result := jsonb_set(v_result, '{shoppingItems}', (
        SELECT jsonb_build_object(
            'totalItems', COUNT(*),
            'checkedToday', COUNT(*) FILTER (WHERE is_checked = true AND checked_at::DATE = v_today),
...
-- Add comment
COMMENT ON FUNCTION public.get_dashboard_summary IS 'Optimized dashboard summary aggregation - replaces 18+ individual queries with single RPC call. Returns flat JSONB matching useDashboardStats.ts structure.';
