...
describe('/api/admin/analytics', () => {
  ...
  describe('GET', () => {
    ...
    it('returns 200 with analytics data for authenticated admin', async () => {
      const { checkGeneralRateLimit } = await import('@/lib/ratelimit');
      const { safeCookiesAsync } = await import('@/lib/utils/safe-cookies');
      const { decryptSessionData, validateSessionData } = await import('@/lib/utils/session-crypto-edge');
      ...
      // Mock all supabaseAdmin.from calls that return data for analytics computation
      const mockOrder = vi.fn().mockReturnValue({
        limit: vi.fn().mockResolvedValue({ data: [], error: null }),
      });
      const mockGte = vi.fn().mockReturnValue({ order: mockOrder });
      const mockSelectWithCount = vi.fn().mockReturnValue({ select: vi.fn().mockReturnValue({ then: vi.fn() }) });
      ...
      const request = new NextRequest('http://localhost/api/admin/analytics?range=7d', {
        method: 'GET',
      });
