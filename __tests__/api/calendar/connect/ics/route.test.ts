...
import { POST, GET, DELETE } from '@/app/api/calendar/connect/ics/route';
...
function makeSuccessSupabase() {
  ...
  const newConnChain = createChainMock({ data: { id: CONNECTION_ID }, error: null });
  ...
  return {
    ...
    from: vi.fn((table: string) => {
      ...
      if (table === 'calendar_connections') {
        fromCallCount++;
        if (fromCallCount === 1) return noActiveConnChain;
        return newConnChain;
      }
...
describe('/api/calendar/connect/ics', () => {
  ...
  describe('POST', () => {
    it('returns 429 when rate limit exceeded', async () => {
      ...
      const req = new NextRequest('http://localhost/api/calendar/connect/ics', {
        method: 'POST',
    ...
    it('returns 400 when body fails Zod validation', async () => {
      ...
      const req = new NextRequest('http://localhost/api/calendar/connect/ics', {
        method: 'POST',
        body: JSON.stringify({ space_id: 'bad-uuid', url: 'not-a-url', name: '' }),
        headers: { 'Content-Type': 'application/json' },
      });
      const res = await POST(req);
      const data = await res.json();
    ...
    it('returns 400 when ICS URL validation fails', async () => {
      const { checkGeneralRateLimit } = await import('@/lib/ratelimit');
      const { createClient } = await import('@/lib/supabase/server');
    ...
    it('returns 403 when user is not a space member', async () => {
      ...
      const { icsImportService } = await import('@/lib/services/calendar');
      vi.mocked(checkGeneralRateLimit).mockResolvedValue(mockRateLimitOk);
      const noMemberChain = createChainMock({ data: null, error: { message: 'Not found' } });
      vi.mocked(createClient).mockResolvedValue({
        auth: { getUser: vi.fn().mockResolvedValue({ data: { user: { id: USER_ID } }, error: null }) },
        from: vi.fn().mockReturnValue(noMemberChain),
      } as never);
      vi.mocked(icsImportService.validateICSUrl).mockReturnValue({
        valid: true,
    ...
    it('returns 400 when ICS feed test fails', async () => {
      const { checkGeneralRateLimit } = await import('@/lib/ratelimit');
      const { createClient } = await import('@/lib/supabase/server');
      const { icsImportService } = await import('@/lib/services/calendar');
      vi.mocked(checkGeneralRateLimit).mockResolvedValue(mockRateLimitOk);
      ...
      expect(res.status).toBe(400);
      expect(data.error).toContain('validate');
    });

    it('returns 200 on successful ICS connection', async () => {
      const { checkGeneralRateLimit } = await import('@/lib/ratelimit');
      const { createClient } = await import('@/lib/supabase/server');
      const { icsImportService } = await import('@/lib/services/calendar');
      vi.mocked(checkGeneralRateLimit).mockResolvedValue(mockRateLimitOk);
  ...
  describe('GET', () => {
    ...
    it('returns 200 with connections list', async () => {
      ...
      const { createClient } = await import('@/lib/supabase/server');
      vi.mocked(checkGeneralRateLimit).mockResolvedValue(mockRateLimitOk);
      const connChain = createChainMock({ data: [{ id: CONNECTION_ID, provider: 'ics' }], error: null });
      vi.mocked(createClient).mockResolvedValue({
        auth: { getUser: vi.fn().mockResolvedValue({ data: { user: { id: USER_ID } }, error: null }) },
        from: vi.fn().mockReturnValue(connChain),
      } as never);

      const req = new NextRequest(`http://localhost/api/calendar/connect/ics?space_id=${SPACE_ID}`);
      const res = await GET(req);
      const data = await res.json();
  ...
  describe('DELETE', () => {
    ...
    it('returns 404 when connection is not found', async () => {
      ...
      const req = new NextRequest(`http://localhost/api/calendar/connect/ics?connection_id=${CONNECTION_ID}`, {
        method: 'DELETE',
      });
      const res = await DELETE(req);
      const data = await res.json();
    ...
    it('returns 200 on successful deletion', async () => {
      ...
      vi.mocked(createClient).mockResolvedValue({
        ...
        from: vi.fn().mockReturnValue({
          ...createChainMock({ data: { id: CONNECTION_ID, user_id: USER_ID, provider: 'ics' }, error: null }),
          select: vi.fn().mockReturnThis(),
          eq: vi.fn().mockReturnThis(),
          single: vi.fn().mockResolvedValue({ data: { id: CONNECTION_ID, user_id: USER_ID, provider: 'ics' }, error: null }),
