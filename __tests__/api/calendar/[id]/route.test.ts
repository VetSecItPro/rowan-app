import { describe, it, expect, vi, beforeEach } from 'vitest';
import { NextRequest } from 'next/server';
import { GET, PATCH, DELETE } from '@/app/api/calendar/[id]/route';

vi.mock('@/lib/supabase/server', () => ({
  createClient: vi.fn(),
}));

vi.mock('@/lib/services/calendar-service', () => ({
  calendarService: {
    getEventById: vi.fn(),
    updateEvent: vi.fn(),
    deleteEvent: vi.fn(),
  },
}));

vi.mock('@/lib/services/authorization-service', () => ({
  verifyResourceAccess: vi.fn(),
}));

vi.mock('@/lib/ratelimit', () => ({
  checkGeneralRateLimit: vi.fn(),
}));

vi.mock('@/lib/ratelimit-fallback', () => ({
  extractIP: vi.fn(() => '127.0.0.1'),
}));

vi.mock('@/lib/validations/calendar-event-schemas', () => ({
  updateCalendarEventSchema: {
    parse: vi.fn(),
  },
}));

vi.mock('@/lib/sanitize', () => ({
  sanitizePlainText: vi.fn((text) => text),
}));

vi.mock('@sentry/nextjs', () => ({
  captureException: vi.fn(),
}));

vi.mock('@/lib/sentry-utils', () => ({
  setSentryUser: vi.fn(),
}));

vi.mock('@/lib/logger', () => ({
  logger: {
    error: vi.fn(),
    warn: vi.fn(),
    info: vi.fn(),
  },
}));

const EVENT_ID = '00000000-0000-4000-8000-000000000010';
const USER_ID = '00000000-0000-4000-8000-000000000001';
const SPACE_ID = '00000000-0000-4000-8000-000000000002';

function makeParams(id: string) {
  return { params: Promise.resolve({ id }) };
}

function mockRateLimitOk() {
  return { success: true, limit: 60, remaining: 59, reset: Date.now() + 60000 };
}

function mockAuthUser(userId = USER_ID) {
  return {
    auth: {
      getUser: vi.fn().mockResolvedValue({
        data: { user: { id: userId } },
        error: null,
      }),
    },
  };
}

describe('/api/calendar/[id]', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  describe('GET', () => {
    it('should return 429 when rate limit is exceeded', async () => {
      const { checkGeneralRateLimit } = await import('@/lib/ratelimit');
      vi.mocked(checkGeneralRateLimit).mockResolvedValue({
        success: false,
        limit: 60,
        remaining: 0,
        reset: Date.now() + 60000,
      });

      const request = new NextRequest(`http://localhost/api/calendar/${EVENT_ID}`, { method: 'GET' });
      const response = await GET(request, makeParams(EVENT_ID));
      const data = await response.json();

      expect(response.status).toBe(429);
      expect(data.error).toContain('Too many requests');
    });

    it('should return 401 when not authenticated', async () => {
      const { checkGeneralRateLimit } = await import('@/lib/ratelimit');
      const { createClient } = await import('@/lib/supabase/server');

      vi.mocked(checkGeneralRateLimit).mockResolvedValue(mockRateLimitOk());
      vi.mocked(createClient).mockResolvedValue({
        auth: {
          getUser: vi.fn().mockResolvedValue({ data: { user: null }, error: { message: 'Not authenticated' } }),
        },
      } as any);

      const request = new NextRequest(`http://localhost/api/calendar/${EVENT_ID}`, { method: 'GET' });
      const response = await GET(request, makeParams(EVENT_ID));
      const data = await response.json();

      expect(response.status).toBe(401);
      expect(data.error).toBe('Unauthorized');
    });

    it('should return 404 when event is not found', async () => {
      const { checkGeneralRateLimit } = await import('@/lib/ratelimit');
      const { createClient } = await import('@/lib/supabase/server');
      const { calendarService } = await import('@/lib/services/calendar-service');

      vi.mocked(checkGeneralRateLimit).mockResolvedValue(mockRateLimitOk());
      vi.mocked(createClient).mockResolvedValue(mockAuthUser() as any);
      vi.mocked(calendarService.getEventById).mockResolvedValue(null);

      const request = new NextRequest(`http://localhost/api/calendar/${EVENT_ID}`, { method: 'GET' });
      const response = await GET(request, makeParams(EVENT_ID));
      const data = await response.json();

      expect(response.status).toBe(404);
      expect(data.error).toBe('Calendar event not found');
    });

    it('should return 403 when user lacks access to the event', async () => {
      const { checkGeneralRateLimit } = await import('@/lib/ratelimit');
      const { createClient } = await import('@/lib/supabase/server');
      const { calendarService } = await import('@/lib/services/calendar-service');
      const { verifyResourceAccess } = await import('@/lib/services/authorization-service');

      vi.mocked(checkGeneralRateLimit).mockResolvedValue(mockRateLimitOk());
      vi.mocked(createClient).mockResolvedValue(mockAuthUser() as any);
      vi.mocked(calendarService.getEventById).mockResolvedValue({ id: EVENT_ID, space_id: SPACE_ID } as any);
      vi.mocked(verifyResourceAccess).mockRejectedValue(new Error('Access denied'));

      const request = new NextRequest(`http://localhost/api/calendar/${EVENT_ID}`, { method: 'GET' });
      const response = await GET(request, makeParams(EVENT_ID));
      const data = await response.json();

      expect(response.status).toBe(403);
      expect(data.error).toBe('You do not have access to this calendar event');
    });

    it('should return the calendar event successfully', async () => {
      const { checkGeneralRateLimit } = await import('@/lib/ratelimit');
      const { createClient } = await import('@/lib/supabase/server');
      const { calendarService } = await import('@/lib/services/calendar-service');
      const { verifyResourceAccess } = await import('@/lib/services/authorization-service');

      const mockEvent = { id: EVENT_ID, title: 'Team Meeting', space_id: SPACE_ID };

      vi.mocked(checkGeneralRateLimit).mockResolvedValue(mockRateLimitOk());
      vi.mocked(createClient).mockResolvedValue(mockAuthUser() as any);
      vi.mocked(calendarService.getEventById).mockResolvedValue(mockEvent as any);
      vi.mocked(verifyResourceAccess).mockResolvedValue(undefined);

      const request = new NextRequest(`http://localhost/api/calendar/${EVENT_ID}`, { method: 'GET' });
      const response = await GET(request, makeParams(EVENT_ID));
      const data = await response.json();

      expect(response.status).toBe(200);
      expect(data.success).toBe(true);
      expect(data.data).toEqual(mockEvent);
    });
  });

  describe('PATCH', () => {
    it('should return 401 when not authenticated', async () => {
      const { checkGeneralRateLimit } = await import('@/lib/ratelimit');
      const { createClient } = await import('@/lib/supabase/server');

      vi.mocked(checkGeneralRateLimit).mockResolvedValue(mockRateLimitOk());
      vi.mocked(createClient).mockResolvedValue({
        auth: {
          getUser: vi.fn().mockResolvedValue({ data: { user: null }, error: { message: 'Not authenticated' } }),
        },
      } as any);

      const request = new NextRequest(`http://localhost/api/calendar/${EVENT_ID}`, {
        method: 'PATCH',
        body: JSON.stringify({ title: 'Updated Meeting' }),
      });
      const response = await PATCH(request, makeParams(EVENT_ID));
      const data = await response.json();

      expect(response.status).toBe(401);
      expect(data.error).toBe('Unauthorized');
    });

    it('should return 404 when event is not found', async () => {
      const { checkGeneralRateLimit } = await import('@/lib/ratelimit');
      const { createClient } = await import('@/lib/supabase/server');
      const { calendarService } = await import('@/lib/services/calendar-service');

      vi.mocked(checkGeneralRateLimit).mockResolvedValue(mockRateLimitOk());
      vi.mocked(createClient).mockResolvedValue(mockAuthUser() as any);
      vi.mocked(calendarService.getEventById).mockResolvedValue(null);

      const request = new NextRequest(`http://localhost/api/calendar/${EVENT_ID}`, {
        method: 'PATCH',
        body: JSON.stringify({ title: 'Updated Meeting' }),
      });
      const response = await PATCH(request, makeParams(EVENT_ID));
      const data = await response.json();

      expect(response.status).toBe(404);
      expect(data.error).toBe('Calendar event not found');
    });

    it('should return 403 when user lacks access', async () => {
      const { checkGeneralRateLimit } = await import('@/lib/ratelimit');
      const { createClient } = await import('@/lib/supabase/server');
      const { calendarService } = await import('@/lib/services/calendar-service');
      const { verifyResourceAccess } = await import('@/lib/services/authorization-service');

      vi.mocked(checkGeneralRateLimit).mockResolvedValue(mockRateLimitOk());
      vi.mocked(createClient).mockResolvedValue(mockAuthUser() as any);
      vi.mocked(calendarService.getEventById).mockResolvedValue({ id: EVENT_ID, space_id: SPACE_ID } as any);
      vi.mocked(verifyResourceAccess).mockRejectedValue(new Error('Access denied'));

      const request = new NextRequest(`http://localhost/api/calendar/${EVENT_ID}`, {
        method: 'PATCH',
        body: JSON.stringify({ title: 'Updated Meeting' }),
      });
      const response = await PATCH(request, makeParams(EVENT_ID));
      const data = await response.json();

      expect(response.status).toBe(403);
      expect(data.error).toBe('You do not have access to this calendar event');
    });

    it('should return 400 for invalid update body', async () => {
      const { checkGeneralRateLimit } = await import('@/lib/ratelimit');
      const { createClient } = await import('@/lib/supabase/server');
      const { calendarService } = await import('@/lib/services/calendar-service');
      const { verifyResourceAccess } = await import('@/lib/services/authorization-service');
      const { updateCalendarEventSchema } = await import('@/lib/validations/calendar-event-schemas');
      const { ZodError, ZodIssueCode } = await import('zod');

      vi.mocked(checkGeneralRateLimit).mockResolvedValue(mockRateLimitOk());
      vi.mocked(createClient).mockResolvedValue(mockAuthUser() as any);
      vi.mocked(calendarService.getEventById).mockResolvedValue({ id: EVENT_ID, space_id: SPACE_ID } as any);
      vi.mocked(verifyResourceAccess).mockResolvedValue(undefined);
      vi.mocked(updateCalendarEventSchema.parse).mockImplementation(() => {
        throw new ZodError([
          {
            code: ZodIssueCode.invalid_type,
            expected: 'string',
            received: 'number',
            path: ['title'],
            message: 'Expected string, received number',
          },
        ]);
      });

      const request = new NextRequest(`http://localhost/api/calendar/${EVENT_ID}`, {
        method: 'PATCH',
        body: JSON.stringify({ title: 999 }),
      });
      const response = await PATCH(request, makeParams(EVENT_ID));
      const data = await response.json();

      expect(response.status).toBe(400);
      expect(data.error).toBe('Validation failed');
    });

    it('should update calendar event successfully', async () => {
      const { checkGeneralRateLimit } = await import('@/lib/ratelimit');
      const { createClient } = await import('@/lib/supabase/server');
      const { calendarService } = await import('@/lib/services/calendar-service');
      const { verifyResourceAccess } = await import('@/lib/services/authorization-service');
      const { updateCalendarEventSchema } = await import('@/lib/validations/calendar-event-schemas');

      const updatedEvent = { id: EVENT_ID, title: 'Updated Meeting', space_id: SPACE_ID };

      vi.mocked(checkGeneralRateLimit).mockResolvedValue(mockRateLimitOk());
      vi.mocked(createClient).mockResolvedValue(mockAuthUser() as any);
      vi.mocked(calendarService.getEventById).mockResolvedValue({ id: EVENT_ID, space_id: SPACE_ID } as any);
      vi.mocked(verifyResourceAccess).mockResolvedValue(undefined);
      vi.mocked(updateCalendarEventSchema.parse).mockReturnValue(undefined);
      vi.mocked(calendarService.updateEvent).mockResolvedValue(updatedEvent as any);

      const request = new NextRequest(`http://localhost/api/calendar/${EVENT_ID}`, {
        method: 'PATCH',
        body: JSON.stringify({ title: 'Updated Meeting' }),
      });
      const response = await PATCH(request, makeParams(EVENT_ID));
      const data = await response.json();

      expect(response.status).toBe(200);
      expect(data.success).toBe(true);
      expect(data.data.title).toBe('Updated Meeting');
    });
  });

  describe('DELETE', () => {
    it('should return 401 when not authenticated', async () => {
      const { checkGeneralRateLimit } = await import('@/lib/ratelimit');
      const { createClient } = await import('@/lib/supabase/server');

      vi.mocked(checkGeneralRateLimit).mockResolvedValue(mockRateLimitOk());
      vi.mocked(createClient).mockResolvedValue({
        auth: {
          getUser: vi.fn().mockResolvedValue({ data: { user: null }, error: { message: 'Not authenticated' } }),
        },
      } as any);

      const request = new NextRequest(`http://localhost/api/calendar/${EVENT_ID}`, { method: 'DELETE' });
      const response = await DELETE(request, makeParams(EVENT_ID));
      const data = await response.json();

      expect(response.status).toBe(401);
      expect(data.error).toBe('Unauthorized');
    });

    it('should return 404 when event is not found', async () => {
      const { checkGeneralRateLimit } = await import('@/lib/ratelimit');
      const { createClient } = await import('@/lib/supabase/server');
      const { calendarService } = await import('@/lib/services/calendar-service');

      vi.mocked(checkGeneralRateLimit).mockResolvedValue(mockRateLimitOk());
      vi.mocked(createClient).mockResolvedValue(mockAuthUser() as any);
      vi.mocked(calendarService.getEventById).mockResolvedValue(null);

      const request = new NextRequest(`http://localhost/api/calendar/${EVENT_ID}`, { method: 'DELETE' });
      const response = await DELETE(request, makeParams(EVENT_ID));
      const data = await response.json();

      expect(response.status).toBe(404);
      expect(data.error).toBe('Calendar event not found');
    });

    it('should return 403 when user lacks access', async () => {
      const { checkGeneralRateLimit } = await import('@/lib/ratelimit');
      const { createClient } = await import('@/lib/supabase/server');
      const { calendarService } = await import('@/lib/services/calendar-service');
      const { verifyResourceAccess } = await import('@/lib/services/authorization-service');

      vi.mocked(checkGeneralRateLimit).mockResolvedValue(mockRateLimitOk());
      vi.mocked(createClient).mockResolvedValue(mockAuthUser() as any);
      vi.mocked(calendarService.getEventById).mockResolvedValue({ id: EVENT_ID, space_id: SPACE_ID } as any);
      vi.mocked(verifyResourceAccess).mockRejectedValue(new Error('Access denied'));

      const request = new NextRequest(`http://localhost/api/calendar/${EVENT_ID}`, { method: 'DELETE' });
      const response = await DELETE(request, makeParams(EVENT_ID));
      const data = await response.json();

      expect(response.status).toBe(403);
      expect(data.error).toBe('You do not have access to this calendar event');
    });

    it('should delete calendar event successfully', async () => {
      const { checkGeneralRateLimit } = await import('@/lib/ratelimit');
      const { createClient } = await import('@/lib/supabase/server');
      const { calendarService } = await import('@/lib/services/calendar-service');
      const { verifyResourceAccess } = await import('@/lib/services/authorization-service');

      vi.mocked(checkGeneralRateLimit).mockResolvedValue(mockRateLimitOk());
      vi.mocked(createClient).mockResolvedValue(mockAuthUser() as any);
      vi.mocked(calendarService.getEventById).mockResolvedValue({ id: EVENT_ID, space_id: SPACE_ID } as any);
      vi.mocked(verifyResourceAccess).mockResolvedValue(undefined);
      vi.mocked(calendarService.deleteEvent).mockResolvedValue(undefined);

      const request = new NextRequest(`http://localhost/api/calendar/${EVENT_ID}`, { method: 'DELETE' });
      const response = await DELETE(request, makeParams(EVENT_ID));
      const data = await response.json();

      expect(response.status).toBe(200);
      expect(data.success).toBe(true);
      expect(data.message).toBe('Calendar event deleted successfully');
    });
  });
});
