...
describe('/api/shopping', () => {
  ...
  describe('GET', () => {
    it('should return 429 when rate limit exceeded', async () => {
      ...
      const request = new NextRequest('http://localhost/api/shopping?space_id=space-123', {
        method: 'GET',
      });
    ...
    it('should return shopping lists successfully', async () => {
      const { checkGeneralRateLimit } = await import('@/lib/ratelimit');
      const { createClient } = await import('@/lib/supabase/server');
      const { verifySpaceAccess } = await import('@/lib/services/authorization-service');
      ...
      const mockLists = [
        { id: 'list-1', name: 'Groceries', items: [] },
        { id: 'list-2', name: 'Hardware Store', items: [] },
      ];

      vi.mocked(shoppingService.getLists).mockResolvedValue(mockLists as any);
  ...
  describe('POST', () => {
    ...
    it('should return 429 when usage limit exceeded', async () => {
      ...
      vi.mocked(checkUsageLimit).mockResolvedValue({
        allowed: false,
        message: 'Daily shopping update limit reached',
        currentUsage: 20,
        limit: 20,
        remaining: 0,
      });
      ...
      expect(response.status).toBe(429);
      expect(data.error).toBe('Daily shopping update limit reached');
    ...
    it('should create shopping list successfully', async () => {
      ...
      vi.mocked(shoppingService.createList).mockResolvedValue(mockList as any);
      ...
      expect(trackUsage).toHaveBeenCalledWith('00000000-0000-4000-8000-000000000001', 'shopping_list_updates');
    });
